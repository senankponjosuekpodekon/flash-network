...
export default function WithdrawPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setResult(null);
    try {
      const res = await api.withdraw(to, Number(amount));
      setResult(res);
      showToast("Withdrawal initiated! Balance debited.");
      setTo("");
      setAmount("");
    } catch (err) {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">Withdraw FLASH</h1>

      <div className="card max-w-lg">
        <p className="text-sm text-slate-400 mb-4">
          Withdraw your internal FLASH balance to an external TRON wallet. This is an on-chain transaction.
        </p>
        ...
        {result && (
          <div className="mt-4 p-4 bg-green-950/50 border border-green-900 rounded-lg">
            <div className="text-sm text-green-400 font-medium">Withdrawal initiated!</div>
            <div className="text-xs font-mono text-slate-400 mt-1 break-all">TXID: {result.txid}</div>
            <div className="text-xs text-slate-400 mt-1">
              Amount: {result.amount} → {result.to}
            </div>
          </div>
        )}
      </div>
